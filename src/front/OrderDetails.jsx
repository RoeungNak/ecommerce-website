...
import Layout from "../pages/user_profile/LayoutProfile"; // user layout
...
const OrderDetails = () => {
  ...
  const fetchOrder = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${apiUrl}/user/order/${id}`, {
        method: "GET",
        headers: {
    ...
    <Layout>
      <div className="max-w-4xl mx-auto p-6 bg-white shadow rounded-lg">
        ...
        <div className="flex gap-1 font-semibold items-center mb-6">
          ...
          <span>/</span>
          <Link to="/account/orders" className="hover:text-red-500">
            My Orders
          </Link>
          <span>/</span>
              ...
              {order.items.map((item) => (
                <tr key={item.id} className="border-t">
                  ...
                  <td className="px-4 py-2 text-center">
                    {formatUSD(item.product?.price || 0)}
                  </td>
                  <td className="px-4 py-2 text-center">
